export default class Resource {
    ...
    /**
     * Performs batch selected by value in the field
     * @param field {string} Column name
     * @param value {[]} The value which element[field] must be equal
     * @param page {int} Page index starting from 0
     * @param perPage {int} Rows per page
     * @param orderBy? {string} Column name to sort with (ASC)
     * @returns {Promise}
     */
    getBatchByElementValue(field, value, page = 0, perPage = 5, orderBy = 'id') {
        return this._promise(
            this._paginate(
                this._orderBy(
                    this.data.filter(el => {
